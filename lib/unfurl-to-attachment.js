...
module.exports = async function unfurlToAttachment (url) {
  ...
  if (data.other && data.other.icon) {
    ...
    if (!parseIconUrl.protocol && !parseIconUrl.host) baseUrl = parsedUrl.protocol + '//' + parsedUrl.host
    else if (!parseIconUrl.protocol && parseIconUrl.host) baseUrl = parsedUrl.protocol + '//'
  }
  if (baseUrl) {
